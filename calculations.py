...
# generate a list of abilities, to be selected in a GUI
def list_of_abilities() -> list:
    abilities_df = pd.read_csv('Ability_information.csv', index_col='name')
    ability_list = list(abilities_df.index)
    return ability_list
...
# generate list of buffs
# each item in this list will have a separate selection menu in the GUI
def zeroed_dict_of_buffs() -> dict:
    buff_list = ['Critical Strike Chance', 'Tsunami', 'Natural Instinct', 'Ring of Vigour', 'Invigorating']
    # will be populated with values once the user selects them in the GUI
    buff_dict = {buff: 0 for buff in buff_list}
    return buff_dict
